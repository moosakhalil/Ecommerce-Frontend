...
const IssueReportModal = ({ orderId, productIndex, product, onClose, onSuccess }) => {
  ...
  const issueCategories = [
    { group: "Product Condition Issues", color: "#dc3545", items: [
      { value: "product_broken_damaged", label: "Product broken or damaged" },
      { value: "scratched_dented_cracked", label: "Scratched, dented, or cracked" },
      { value: "leaking", label: "Leaking (for liquids)" },
      { value: "expired_close_to_expiry", label: "Expired or close to expiration date" },
      { value: "defective_not_working", label: "Defective / not working" },
      { value: "poor_quality_manufacturing_fault", label: "Poor quality or manufacturing fault" },
    ]},
    { group: "Quantity Issues", color: "#fd7e14", items: [
      { value: "missing_items", label: "Missing items" },
      { value: "wrong_quantity", label: "Wrong quantity (too many / too few)" },
      { value: "incomplete_set", label: "Incomplete set or kit" },
      { value: "extra_items_by_mistake", label: "Extra items included by mistake" },
    ]},
    { group: "Product Specification Issues", color: "#6f42c1", items: [
      ...
      { value: "wrong_flavor_scent", label: "Wrong flavor / scent (if applicable)" },
    ]},
    { group: "Packaging Issues", color: "#17a2b8", items: [
      { value: "packaging_damaged", label: "Packaging damaged" },
      { value: "packaging_opened_tampered", label: "Packaging opened or tampered with" },
    ...
    { group: "Labeling & Documentation Issues", color: "#6c757d", items: [
      { value: "wrong_barcode", label: "Wrong barcode" },
      { value: "missing_barcode", label: "Missing barcode" },
      { value: "incorrect_product_description", label: "Incorrect product description on label" },
      { value: "missing_instructions_manual", label: "Missing instructions or manual" },
      { value: "missing_warranty_documents", label: "Missing warranty card or documents" },
    ]},
    { group: "Order & Shipping Issues", color: "#20c997", items: [
      { value: "wrong_product_sent", label: "Wrong product sent" },
      { value: "mixed_products", label: "Mixed products in one order" },
      { value: "incorrect_customer_order", label: "Incorrect customer order" },
      { value: "delivery_delay", label: "Delivery delay" },
      { value: "wrong_location", label: "Product sent to wrong location" },
    ]},
    { group: "Compliance & Safety Issues", color: "#e83e8c", items: [
      { value: "not_meeting_safety_standards", label: "Product not meeting safety standards" },
      { value: "missing_safety_warnings", label: "Missing safety warnings" },
      { value: "incorrect_certification", label: "Incorrect certification or approval marks" },
    ]},
  ];
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const category of selectedIssues) {
        const response = await fetch(
          `${API_BASE_URL}/api/supplier-orders/${orderId}/product-issue/${productIndex}`,
          {
            ...
            body: JSON.stringify({
              category,
              description: selectedIssues.length === 1 ? description : `[${category.replace(/_/g, " ")}] ${description}`,
              reportedBy: "Admin",
            }),
          }
        );
      ...
      if (mediaFiles.length > 0) {
        ...
        await fetch(`${API_BASE_URL}/api/supplier-orders/${orderId}/upload-media`, {
          method: "POST",
          body: formData,
        });
      }
      ...
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={styles.content}>
          ...
          <div style={styles.section}>
            <label style={styles.label}>Select Issue Categories (can select multiple)</label>
          ...
          <div style={styles.section}>
            ...
            >
              ...
              <div style={{ fontWeight: "600", fontSize: "13px" }}>Click to upload photos or videos</div>
        ...
        <div style={styles.footer}>
          <span style={styles.selectedCount}>
            {selectedCount > 0 ? `${selectedCount} issue${selectedCount > 1 ? 's' : ''} selected` : 'No issues selected'}
          </span>
          <div style={styles.footerButtons}>
            ...
            <button
              style={{ ...styles.btn, ...styles.btnPrimary, opacity: uploading || selectedCount === 0 ? 0.7 : 1 }}
              onClick={handleSubmit}
              disabled={uploading || selectedCount === 0}
            >
              {uploading ? "Submitting..." : `Submit ${selectedCount} Issue${selectedCount !== 1 ? 's' : ''}`}
            </button>
          </div>
        </div>
      </div>
