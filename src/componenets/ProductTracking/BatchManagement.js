  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (Array.isArray(productsData)) {
        ...
        setProducts([]);
        console.warn(
          "Products API response format not recognized:",
          productsData,
        );
      }
    } catch (err) {
  ...
  const handleReceiveSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/api/tracking/receive`,
        {
      ...
      if (response.data.success) {
        alert(
          `Successfully created ${response.data.data.items.length} tracking items!\nBatch ID: ${response.data.data.batchId}`,
        );
      ...
      >
        <div className="tracking-container">
          <div className="tracking-header">
            <h1>
              <strong>303.</strong> 📦 Batch Management
            </h1>
            <button
              ...
              {batches.length > 0 ? (
                      ...
                      {batches.map((batch, index) => {
                        ...
                        return (
                          <tr key={index}>
                            <td>
                              <strong>
                                {batch.batchId || batch._id?.batchId}
                              </strong>
                            </td>
                            <td>
                              <div>
                ...
                <div className="tracking-empty-state">
                  ...
                  <p className="empty-text">
                    Start by receiving a shipment from a supplier.
                  </p>
                  <button
                    className="btn-primary"
                    onClick={() => setShowReceiveModal(true)}
                  ...
                  {expiringBatches.map((batch, index) => {
                    ...
                    return (
                      <div
                        ...
                        style={{ borderLeftColor: getExpiryColor(daysLeft) }}
                      >
                        ...
                        <div className="form-group">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setReceiveForm({
                                ...receiveForm,
                                supplierOrderNumber: e.target.value,
                              })
                            }
                          />
                        </div>
