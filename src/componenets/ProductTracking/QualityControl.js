...
const QualityControl = () => {
  ...
  const [pendingData, setPendingData] = useState({ inProgress: [], needsCheck: [], totalPending: 0 });
  ...
  const handleSubmitCheck = async (e) => {
    ...
    try {
      ...
      alert("Failed to submit quality check: " + (err.response?.data?.message || err.message));
    }
  };
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="tracking-stats-grid" style={{ marginBottom: "24px" }}>
        <div className="tracking-stat-card" style={{ borderLeftColor: "#f59e0b" }}>
        ...
        <div className="tracking-stat-card" style={{ borderLeftColor: "#10b981" }}>
          <div className="stat-icon" style={{ backgroundColor: "#d1fae5", color: "#10b981" }}>
            ✅
          </div>
          <div className="stat-content">
            <h3 className="stat-value">
              {history.filter(h => h.qualityChecks?.some(q => q.status === "passed")).length}
            </h3>
            <p className="stat-title">Passed (7 days)</p>
          </div>
        </div>

        <div className="tracking-stat-card" style={{ borderLeftColor: "#ef4444" }}>
          <div className="stat-icon" style={{ backgroundColor: "#fee2e2", color: "#ef4444" }}>
            ❌
          </div>
          <div className="stat-content">
            <h3 className="stat-value">
              {history.filter(h => h.qualityChecks?.some(q => q.status === "failed")).length}
            </h3>
      ...
      {activeTab === "pending" && (
        <div className="tracking-section">
          {pendingData.needsCheck?.length > 0 || pendingData.inProgress?.length > 0 ? (
            <>
                  ...
                  <div className="activity-list" style={{ marginBottom: "24px" }}>
                    {pendingData.inProgress.map((item, index) => (
                      <div key={index} className="activity-item">
                        <div className="activity-status" style={{ backgroundColor: "#f59e0b" }}></div>
                        <div className="activity-content">
                          <p className="activity-text">
                            <strong>{item.uti}</strong> - {item.productName}
                          </p>
                          <p className="activity-meta">
                            Batch: {item.batchId} • Location: {item.currentLocation?.warehouseZone || "Unknown"}
                          </p>
                        </div>
                        <button className="btn-primary" onClick={() => handleStartCheck(item)}>
                          Continue Check
                        </button>
                      </div>
                    ))}
              ...
              <div className="activity-list">
                {pendingData.needsCheck?.map((item, index) => (
                  <div key={index} className="activity-item">
                    <div className="activity-status" style={{ backgroundColor: "#3b82f6" }}></div>
                    <div className="activity-content">
                      <p className="activity-text">
                        <strong>{item.uti}</strong> - {item.productName}
                      </p>
                      <p className="activity-meta">
                        Batch: {item.batchId} • Received: {formatDate(item.receivedDate)}
                      </p>
                    </div>
                    <button className="btn-primary" onClick={() => handleStartCheck(item)}>
                      Start Check
              ...
              <table className="tracking-table">
                ...
                <tbody>
                  {history.map((item, index) => {
                    const latestCheck = item.qualityChecks?.[item.qualityChecks.length - 1];
                    return (
                      <tr key={index}>
                        <td><strong>{item.uti}</strong></td>
                        ...
                        <td style={{ textTransform: "capitalize" }}>{latestCheck?.condition}</td>
      ...
      {showCheckModal && selectedItem && (
        <div className="tracking-modal-overlay" onClick={() => setShowCheckModal(false)}>
          <div className="tracking-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: "600px" }}>
            <div className="tracking-modal-header">
              <h2>✅ Quality Inspection</h2>
              <button className="modal-close-btn" onClick={() => setShowCheckModal(false)}>×</button>
            </div>
            ...
            <form onSubmit={handleSubmitCheck}>
              <div className="tracking-modal-body">
                ...
                <div style={{ background: "#f9fafb", padding: "16px", borderRadius: "8px", marginBottom: "24px" }}>
                  <h4 style={{ margin: "0 0 8px 0" }}>{selectedItem.uti}</h4>
                  <p style={{ margin: 0, color: "#6b7280" }}>
                    {selectedItem.productName} • Batch: {selectedItem.batchId}
                  </p>
                </div>
                ...
                <div className="tracking-form">
                      ...
                      {["passed", "failed", "conditional"].map(status => (
                        <label
                          ...
                          style={{
                            ...
                            borderRadius: "8px",
                            cursor: "pointer",
                            background: checkForm.status === status ? `${getStatusColor(status)}10` : "white",
                          }}
                        >
                          <input
                            type="radio"
                  ...
                  <div className="form-group">
                    ...
                    <select
                      ...
                      onChange={(e) => setCheckForm({ ...checkForm, condition: e.target.value })}
                    >
                  ...
                  {/* Issues */}
                  {(checkForm.status === "failed" || checkForm.status === "conditional") && (
                    <div className="form-group">
                        ...
                        {issueOptions.map(issue => (
                          <label
                            ...
                            style={{
                              ...
                              borderRadius: "6px",
                              cursor: "pointer",
                              background: checkForm.issues.includes(issue) ? "#f3e8ff" : "white",
                              fontSize: "13px",
                            }}
                          >
                            <input
                              ...
                              style={{ display: "none" }}
                            />
                            <span style={{ textTransform: "capitalize" }}>{issue.replace(/_/g, " ")}</span>
                          </label>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) => setCheckForm({ ...checkForm, checkedBy: e.target.value })}
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="tracking-modal-footer">
                <button type="button" className="btn-secondary" onClick={() => setShowCheckModal(false)}>
                  Cancel
                </button>
