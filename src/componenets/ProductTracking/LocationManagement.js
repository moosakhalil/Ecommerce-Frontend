...
const LocationManagement = () => {
  ...
  const [locationData, setLocationData] = useState({ zones: [], locations: [] });
  ...
  const fetchLocationData = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${API_BASE_URL}/api/tracking/locations/summary`);
          ...
          {locationData.zones?.length > 0 ? (
            <div className="tracking-cards-grid">
              {locationData.zones.map((zone, index) => {
                ...
                const utilization = Math.min(100, Math.round((zone.totalUnits / mockCapacity) * 100));
                
                return (
                  <div key={index} className="tracking-card">
                    <div className="tracking-card-header">
                      <div>
                        <h3 className="tracking-card-title">{zone.zone || zone._id}</h3>
                    ...
                    <div className="tracking-card-content" style={{ marginTop: "16px" }}>
                      <div className="tracking-card-row">
                        <span className="tracking-card-label">Total Units:</span>
                        <span className="tracking-card-value" style={{ color: "#7c3aed", fontWeight: 600 }}>
                          {zone.totalUnits}
                        </span>
                      </div>
                      <div className="tracking-card-row">
                        ...
                        >
                          {utilization >= 90 ? "Near Full" : utilization >= 70 ? "Moderate" : "Available"}
                        </span>
                      </div>
                    </div>
                    ...
                    <div style={{ marginTop: "16px" }}>
                      ...
                      <p style={{ textAlign: "center", margin: "8px 0 0 0", fontSize: "12px", color: "#6b7280" }}>
            ...
            <div className="tracking-empty-state">
              ...
              <p className="empty-text">
                Zones will appear here when you receive products and assign locations.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="tracking-section">
          ...
          {locationData.locations?.length > 0 ? (
            ...
            <div className="tracking-empty-state">
              ...
              <p className="empty-text">
                Storage locations will appear here when products are assigned to shelves.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="tracking-stats-grid">
          <div className="tracking-stat-card" style={{ borderLeftColor: "#7c3aed" }}>
            ...
            <div className="stat-content">
              <h3 className="stat-value">{locationData.zones?.length || 0}</h3>
          ...
          <div className="tracking-stat-card" style={{ borderLeftColor: "#10b981" }}>
            ...
            <div className="stat-content">
              <h3 className="stat-value">{locationData.locations?.length || 0}</h3>
              <p className="stat-title">Shelf Locations</p>
            </div>
          </div>

          <div className="tracking-stat-card" style={{ borderLeftColor: "#3b82f6" }}>
            <div className="stat-icon" style={{ backgroundColor: "#dbeafe", color: "#3b82f6" }}>
              📦
            </div>
            <div className="stat-content">
              <h3 className="stat-value">
                {locationData.zones?.reduce((sum, z) => sum + (z.totalUnits || 0), 0) || 0}
              </h3>
