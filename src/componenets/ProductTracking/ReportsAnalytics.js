  ...
  const fetchReports = async () => {
    ...
    try {
      const [inventoryRes, movementRes, dashboardRes] = await Promise.all([
        ...
        axios.get(`${API_BASE_URL}/api/tracking/reports/movements?days=${dateRange}`),
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="tracking-stats-grid" style={{ marginBottom: "24px" }}>
        <div className="tracking-stat-card" style={{ borderLeftColor: "#7c3aed" }}>
          <div className="stat-icon" style={{ backgroundColor: "#f3e8ff", color: "#7c3aed" }}>📦</div>
        ...
        <div className="tracking-stat-card" style={{ borderLeftColor: "#22c55e" }}>
          <div className="stat-icon" style={{ backgroundColor: "#dcfce7", color: "#22c55e" }}>✅</div>
          <div className="stat-content">
        ...
        <div className="tracking-stat-card" style={{ borderLeftColor: "#ef4444" }}>
          <div className="stat-icon" style={{ backgroundColor: "#fee2e2", color: "#ef4444" }}>⚠️</div>
          <div className="stat-content">
            <h3 className="stat-value">{(summary.damaged || 0) + (summary.lost || 0)}</h3>
        ...
        <div className="tracking-section">
          ...
          {inventoryReport.length > 0 ? (
                ...
                <tbody>
                  {inventoryReport.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td style={{ color: item.damaged > 0 ? "#ef4444" : "#6b7280" }}>{item.damaged}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr style={{ background: "#f9fafb", fontWeight: 600 }}>
                    ...
                    <td>{inventoryReport.reduce((sum, i) => sum + i.totalUnits, 0)}</td>
                    <td>{inventoryReport.reduce((sum, i) => sum + i.inStorage, 0)}</td>
                    <td>{inventoryReport.reduce((sum, i) => sum + i.reserved, 0)}</td>
                    <td>{inventoryReport.reduce((sum, i) => sum + i.inTransit, 0)}</td>
                    <td>{inventoryReport.reduce((sum, i) => sum + i.delivered, 0)}</td>
                    <td>{inventoryReport.reduce((sum, i) => sum + i.damaged, 0)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
            ...
            <div className="tracking-empty-state">
              ...
              <p className="empty-text">Start tracking products to see inventory reports.</p>
            </div>
          )}
        </div>
      )}
      ...
      {activeTab === "movements" && (
        <div className="tracking-section">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
            <h2 style={{ margin: 0 }}>Movement Activity (Last {dateRange} Days)</h2>
            <button 
              className="btn-secondary" 
              onClick={() => exportCSV(movementReport.map(m => ({
                date: m._id?.date,
                reason: m._id?.reason,
                count: m.count
              })), "movement_report")}
          ...
          {movementReport.length > 0 ? (
            <>
              ...
              <div className="tracking-cards-grid" style={{ marginBottom: "24px" }}>
                {Object.entries(
                  ...
                  <div key={reason} className="tracking-stat-card" style={{ borderLeftColor: getReasonColor(reason) }}>
                    <div
                      className="stat-icon"
                      style={{ backgroundColor: `${getReasonColor(reason)}20`, color: getReasonColor(reason) }}
                    >
                      📦
                    </div>
                    <div className="stat-content">
                      <h3 className="stat-value">{count}</h3>
                      <p className="stat-title" style={{ textTransform: "capitalize" }}>
                        {reason.replace(/_/g, " ")}
                      </p>
                    </div>
                  </div>
            ...
            <div className="tracking-empty-state">
              ...
              <p className="empty-text">Product movements will appear here.</p>
            </div>
          )}
        </div>
      )}
      ...
      {activeTab === "status" && (
        <div className="tracking-section">
          ...
          {Object.entries(summary).filter(([key]) => key !== "total").length > 0 ? (
            <div className="status-distribution" style={{ maxWidth: "600px" }}>
            ...
            <div className="tracking-empty-state">
              ...
              <p className="empty-text">Start tracking products to see status distribution.</p>
            </div>
          )}
        </div>
      )}
