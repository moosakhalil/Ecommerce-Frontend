    ...
    try {
      ...
      if (response.data.success) {
        ...
        setFormData({
          ...
          serviceDueDate: vehicle.serviceDueDate ? vehicle.serviceDueDate.split('T')[0] : '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.vehicleType === 'truck') {
        if (formData.heightCm) submitData.append('dimensions[heightCm]', formData.heightCm);
        if (formData.widthCm) submitData.append('dimensions[widthCm]', formData.widthCm);
        if (formData.lengthCm) submitData.append('dimensions[lengthCm]', formData.lengthCm);
      }
      ...
      if (formData.yearModel) submitData.append('yearModel', formData.yearModel);
      if (formData.engineSizeCc) submitData.append('engineSizeCc', formData.engineSizeCc);
      if (formData.odometerKm) submitData.append('odometerKm', formData.odometerKm);
      if (formData.serviceDueDate) submitData.append('serviceDueDate', formData.serviceDueDate);
      if (formData.insuranceExpiryDate) submitData.append('insuranceExpiryDate', formData.insuranceExpiryDate);
      if (formData.chassisNumber) submitData.append('chassisNumber', formData.chassisNumber);
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-lg shadow-md p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Document Upload */}
            <div className="border-t pt-6">
              <h3 className="text-lg font-semibold mb-4">Vehicle Documents (Upload new to replace)</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.keys(images).map((imageType) => (
                  <div key={imageType} className="border-2 border-dashed border-gray-300 rounded-lg p-4">
