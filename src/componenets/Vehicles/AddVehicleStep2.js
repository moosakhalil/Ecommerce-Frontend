  ...
  const fetchTemplate = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/api/vehicle-templates/${id}`,
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.yearModel)
        formDataToSend.append("yearModel", formData.yearModel);
      if (formData.engineSizeCc)
        formDataToSend.append("engineSizeCc", formData.engineSizeCc);
      if (formData.odometerKm)
        formDataToSend.append("odometerKm", formData.odometerKm);
      if (formData.serviceDueDate)
        formDataToSend.append("serviceDueDate", formData.serviceDueDate);
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-lg shadow-md p-8">
                ...
                {template.vehicleType === "truck" && template.truckTypeName && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {template.truckTypeName}
                    </span>
                  </div>
                )}
                {template.vehicleType === "scooter" &&
                  template.scooterTypeName && (
                    <div className="flex justify-between">
                ...
                {template.vehicleType === "truck" && template.dimensions && (
                  <>
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {template.dimensions.heightCm}×
                        {template.dimensions.widthCm}×
                        {template.dimensions.lengthCm} cm
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Max Package Length:</span>
                      <span className="font-medium">
                        {template.maxPackageLength} cm
                      </span>
                    </div>
                  </>
                )}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Weight Load limit percentage (kg){" "}
                  <span className="text-red-500">*</span> :{" "}
                  {formData.loadLimitPercent}%
                </label>
                <input
                  type="range"
                ...
                {template.weightMaxKg && formData.loadLimitPercent && (
                  <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      ...
                      {template.weightMaxKg} kg maximum capacity
                    </p>
                  </div>
                )}
              </div>
              ...
              {template.vehicleType === "scooter" && template.maxPackages && (
                <div className="border-t pt-6">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Package Limit % <span className="text-red-500">*</span> :{" "}
                      {formData.packageLimitPercent}%
                    </label>
                    <input
                      type="range"
                    ...
                    <div className="mt-4 p-4 bg-purple-50 border border-purple-200 rounded-lg">
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        ...
                        {template.maxPackages} maximum packages
                      </p>
                    </div>
                  </div>
                </div>
                        ...
                        >
                          ...
                          <span className="text-sm text-gray-600">
                            {images[imageType]
                              ...
                              : `Upload ${imageType.replace(/([A-Z])/g, " $1").toLowerCase()}`}
                          </span>
                        </label>
                      </div>
                    </div>
