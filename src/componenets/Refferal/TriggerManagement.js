...
const TRIGGER_EXPLANATIONS = {
  referral_threshold: {
    ...
    description:
      "Automatically sends a message when a customer has been referred by multiple people.",
    howItWorks:
      "When a potential customer receives referrals from 5+ different people (configurable), the system sends them an encouraging message to make their first purchase. This leverages social proof to increase conversion.",
    example:
      "Sarah has been referred by 5 friends. System sends: 'Hi Sarah, you've been recommended by 5 friends! Use code SARAH123 for 10% off your first order.'",
    benefits: [
      "Increases conversion from referred customers",
  ...
  purchase_conversion: {
    ...
    description:
      "Automatically stops referral messages once a customer makes their first purchase.",
    howItWorks:
      "When a referred customer completes their first order, this trigger automatically disables referral encouragement messages for them to prevent message fatigue.",
    example:
      "John completes his first order → System automatically sets referralMessagesEnabled = false → No more referral messages sent to John.",
    benefits: [
  ...
  inactive_customer: {
    ...
    howItWorks:
      "Monitors days since last purchase. When a customer exceeds the threshold (default: 30 days), sends a re-engagement message with a special offer.",
    example:
      "Mike's last order was 45 days ago → System sends: 'Hi Mike, we miss you! Here's 15% off your next purchase to welcome you back.'",
    benefits: [
      "Recovers lapsed customers",
  ...
  dormant_customer: {
    ...
    description:
      "Targets customers with no activity (messages, cart updates, browsing) for an extended period.",
    howItWorks:
      "Tracks lastInteraction timestamp. When no activity for 60+ days (configurable), sends a stronger re-engagement message with higher incentive.",
    example:
      "Lisa hasn't interacted in 75 days → System sends: 'Hi Lisa, we'd love to have you back! Here's 20% off, valid for 7 days.'",
    benefits: [
      "Last-chance customer recovery",
      "Identifies truly inactive customers",
      "Stronger incentives for dormant users",
    ],
  },
  abandoned_cart: {
    ...
    howItWorks:
      "Monitors cart status. After 24 hours with items in 'cart-not-paid' status, sends reminder with optional incentive.",
    example:
      "Emma added $85 to cart yesterday → System sends: 'Hi Emma, you left items in your cart! Complete your order now for free delivery.'",
    benefits: [
  ...
  first_order_completion: {
    ...
    description:
      "Thanks new customers and encourages referrals while satisfaction is high.",
    howItWorks:
      "Detects first order completion. Sends thank you message 2 hours after delivery with referral code reminder.",
    example:
      "David receives first order → System sends: 'Thank you David! Share your experience with friends using code DAVID789 and earn rewards!'",
    benefits: [
      "Increases referral generation",
      "Strengthens customer relationship",
      "Capitalizes on positive emotions",
    ],
    isNew: true,
  },
  repeat_customer_milestone: {
    ...
    howItWorks:
      "Tracks total completed orders. At configured milestones (5th, 10th, 20th order), sends congratulations with reward.",
    example:
      "Emma places 10th order → System sends: 'Congratulations Emma! Here's $20 credit for your loyalty!'",
    benefits: [
      "Increased retention",
  ...
  high_value_customer: {
    ...
    description:
      "Upgrades customers to VIP status when spending exceeds threshold.",
    howItWorks:
      "Calculates total spending. When crossing $500/$1000/$2000 thresholds, grants VIP status with exclusive benefits.",
    example:
      "Robert spends $750 → System sends: 'You're now a VIP! Enjoy priority service, early access to products, and 5% discount on all orders.'",
    benefits: [
      "Stronger loyalty from top customers",
      "Increased average order value",
      "Word-of-mouth from VIPs",
    ],
    isNew: true,
  },
  referral_success: {
    ...
    howItWorks:
      "Monitors referred customer purchases. When a referral completes first order, notifies original referrer with commission details.",
    example:
      "Your friend Mike makes first purchase → System sends: 'Great news! Mike just purchased. You've earned $5 commission!'",
    benefits: [
      "Motivated referrers",
      "Viral growth",
  ...
  video_upload_reminder: {
    ...
    howItWorks:
      "Identifies customers with referral codes but no videos. After 7 days, sends reminder with instructions.",
    example:
      "You got referral code 10 days ago, no video → System sends: 'Create your 2-minute referral video and start earning rewards!'",
    benefits: [
      "Increased video creation",
      "More referral content",
  ...
  seasonal_promotion: {
    ...
    howItWorks:
      "Administrator configures date range and target audience. System sends promotional message to all eligible customers at scheduled time.",
    example:
      "Holiday week → System sends to all active customers: 'Holiday Sale! 25% off everything this week only!'",
    benefits: [
  ...
  payment_pending: {
    ...
    howItWorks:
      "Monitors orders in 'order-made-not-paid' status. After 2 hours, sends payment reminder with instructions.",
    example:
      "Order created 3 hours ago, no payment → System sends: 'Please upload your payment receipt to confirm your order.'",
    benefits: [
  ...
  delivery_scheduled: {
    ...
    howItWorks:
      "Checks orders with delivery date = tomorrow. Sends reminder 12-18 hours before with driver details and time slot.",
    example:
      "Delivery tomorrow 2-4 PM → System sends tonight: 'Your order will be delivered tomorrow 2-4 PM. Driver: Ahmed (555-1234).'",
    benefits: [
      "Fewer failed deliveries",
      "Better customer experience",
      "Reduced driver time wasted",
    ],
    isNew: true,
  },
  post_delivery_feedback: {
    ...
    howItWorks:
      "Waits 24 hours after delivery completion. Sends feedback request with simple rating interface.",
    example:
      "Order delivered yesterday → System sends: 'How was your delivery? Rate your experience: ⭐⭐⭐⭐⭐'",
    benefits: [
...
const TRIGGER_TYPES = [
  ...
  {
    value: "repeat_customer_milestone",
    label: "Repeat Customer Milestone (NEW)",
  },
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, triggerName: e.target.value })
              }
            ...
            <div className="bg-gray-50 p-4 rounded-lg space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Available placeholders: {"{name}"}, {"{count}"},{" "}
                  {"{referralCode}"}
                </p>
              </div>
...
function TriggerRow({ trigger, onToggle, onEdit, onViewExplanation }) {
  ...
  const borderColor = trigger.isActive
    ? `border-l-4 border-l-${color}-500`
    : "border-l-4 border-l-gray-300";

  return (
    <div
      className={`bg-white ${borderColor} rounded-lg p-4 hover:shadow-md transition-all mb-2`}
    >
      <div className="flex items-center">
        {/* Toggle */}
        <button
        ...
        <div className="flex items-center gap-6 flex-shrink-0 mr-4">
          <div className="text-center w-20">
            <p className="text-xl font-bold text-gray-900">
              {trigger.executionStats?.totalExecutions || 0}
            </p>
            <p className="text-xs text-gray-500">Executions</p>
          </div>
          <div className="text-center w-16">
            <p className="text-xl font-bold text-green-600">
              {trigger.successRate || 0}%
            </p>
...
export default function TriggerManagement() {
  ...
  const handleToggle = async (id) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/triggers/${id}/toggle`,
        {
          method: "POST",
        },
      );
  ...
  const handleSave = async (formData) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/triggers/${editingTrigger._id}`,
        {
          method: "PUT",
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className="mb-4">
          ...
          <p className="text-gray-600 mt-1">
            View and configure automated customer engagement triggers
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-4">
        <div className="flex gap-4">
          <div className="flex-1 relative">
            <Search
              className="absolute left-3 top-1/2 -mt-2.5 text-gray-400"
              size={20}
            />
            <input
              type="text"
