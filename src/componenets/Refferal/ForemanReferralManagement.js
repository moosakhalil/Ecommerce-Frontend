  ...
  >
    ...
    <div>
      ...
      <p style={{ margin: "4px 0 0 0", fontSize: "14px", color: "#475569" }}>
        <strong>Only NEW customers</strong> can be referred.
        <br />
        <strong>Existing customers</strong> already in the system cannot be
        referred.
      </p>
    </div>
  </div>
  ...
  if (loading)
    return (
      <div style={{ color: "#6b7280", padding: "20px" }}>
        Loading commission wallet...
      </div>
    );
  if (!walletData)
    return (
      <div style={{ color: "#6b7280", padding: "20px" }}>No wallet data</div>
    );
  ...
  return (
    <div>
      ...
      >
        <div style={summaryCardStyle}>
          ...
          <strong style={{ fontSize: "20px", color: "#1f2937" }}>
            Rs. {walletData.totalCredits?.toFixed(2) || "0.00"}
          </strong>
        </div>
        <div style={summaryCardStyle}>
          <span style={{ color: "#dc2626" }}>Total Used</span>
          <strong style={{ fontSize: "20px", color: "#1f2937" }}>
            Rs. {walletData.totalDebits?.toFixed(2) || "0.00"}
          </strong>
        </div>
        <div style={summaryCardStyle}>
          <span style={{ color: "#2563eb" }}>Available Balance</span>
          <strong style={{ fontSize: "20px", color: "#1f2937" }}>
            Rs. {walletData.currentBalance?.toFixed(2) || "0.00"}
          </strong>
        </div>
      </div>
      ...
      >
        ...
        <span>
          {showFullList ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
        </span>
      </div>
      ...
      {result && (
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <div>
              <strong
                style={{ color: result.canRefer ? "#16a34a" : "#dc2626" }}
              >
                {result.canRefer
                  ? "✅ Can Be Referred"
                  : "❌ Cannot Be Referred"}
...
export default function ForemanReferralManagement() {
  ...
  const fetchReferrals = async () => {
    try {
      setLoading(true);
      const response = await fetch(
        `${API_BASE_URL}/api/foreman-customers/referrals/all`,
      );
      const data = await response.json();
      ...
      >
        ...
        <div style={{ marginBottom: "24px" }}>
          ...
          <p style={{ color: "#6b7280" }}>
            Manage customer referrals with 5-day rule validation and commission
            tracking
          </p>
        </div>
        ...
        >
          <button
            ...
            style={{
              padding: "10px 20px",
              backgroundColor:
                activeTab === "referrals" ? "#2563eb" : "#f1f5f9",
        ...
        {activeTab === "wallet" && (
          <div>
            ...
            {selectedCustomer ? (
              ...
              >
                Select a customer from the referrals list to view their
                commission wallet
              </div>
            )}
          </div>
