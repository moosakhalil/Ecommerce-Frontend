...
function RejectionModal({ video, onClose, onSubmit, updating }) {
  ...
  const REJECTION_REASONS = [
    ...
    {
      value: "video_quality_unclear",
      label:
        "Video quality is unclear (blurred, low resolution, or poor lighting)",
    },
    {
      value: "audio_not_clear",
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
        ...
        <p className="text-sm text-gray-600 mb-4">
          Your referral video was rejected due to one or more of the following
          reasons
          <span className="text-gray-400"> [Reason appears here]</span>, Please
          review the guidelines and upload a new video addressing the mentioned
          issues
        </p>
  ...
  const scrollToBottom = () => {
    if (videoListRef.current) {
      videoListRef.current.scrollTo({
        top: videoListRef.current.scrollHeight,
        behavior: "smooth",
      });
    }
  };
            ...
            >
              ...
              <div className="p-4 border-b">
                ...
                <div className="flex gap-1 text-xs flex-wrap">
                  ...
                  >
                    ...
                    {
                      allVideos.filter(
                        (v) => v.status === "unverified" || !v.status,
                      ).length
                    }
                    )
                  </button>
                  ...
                  >
                    Manager (
                    {allVideos.filter((v) => v.status === "manager").length})
                  </button>
                  <button
                  ...
                  >
                    Verified (
                    {allVideos.filter((v) => v.status === "verified").length})
                  </button>
                  <button
                  ...
                  >
                    Not Passed (
                    {allVideos.filter((v) => v.status === "not_passed").length})
                  </button>
                </div>
              </div>
              ...
              {loading ? (
                ...
                <>
                  ...
                  <div
                    ref={videoListRef}
                    className="divide-y overflow-y-auto flex-1"
                  >
