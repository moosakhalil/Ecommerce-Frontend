...
function ChatbotControl() {
  ...
  const handleToggle = async (id, currentStatus) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/chatbot-monitor/${id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleToggleAll = async (enable) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/chatbot-monitor/toggle-all`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
        ...
        <div className="p-6 max-w-6xl mx-auto">
          ...
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
              ...
              <strong>115.</strong> Control Chatbot
            </h1>
            <p className="text-gray-600 mt-2">
              Monitor all chatbot messages by receiving copies on selected phone
              numbers
            </p>
          </div>
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p
                    className={`text-lg font-bold ${stats?.isMonitoringActive ? "text-green-600" : "text-gray-400"}`}
                  >
                    ...
                    return (
                      <tr key={monitor._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 font-mono text-gray-900">
                          {monitor.countryCode}{" "}
                          {monitor.phoneNumber.slice(
                            monitor.countryCode.replace("+", "").length,
                          )}
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <button
                            onClick={() =>
                              handleToggle(monitor._id, monitor.isActive)
                            }
