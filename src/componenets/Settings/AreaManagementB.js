...
const AreaManagementB = () => {
  ...
  const fetchLargerStates = async (islandId) => {
    try {
      const res = await axios.get(
        `${API_BASE}/larger-states?islandId=${islandId}`,
      );
  ...
  const fetchRegencies = async (largerStateId) => {
    try {
      const res = await axios.get(
        `${API_BASE}/regencies?largerStateId=${largerStateId}`,
      );
  ...
  const handleAddArea = async () => {
    ...
    try {
      ...
      if (islandMode === "new" && newIsland) {
        const res = await axios.post(`${API_BASE}/islands`, {
          name: newIsland,
        });
            ...
            <div className="bg-green-100 border border-green-200 text-green-800 rounded-lg p-4 mb-6">
              ...
              <div className="text-sm">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2">
                  <div>
                    🏝️ Islands: {uploadResult.stats.islands.created} new
                  </div>
                  <div>📍 States: {uploadResult.stats.states.created} new</div>
                  <div>
                    🏘️ Regencies: {uploadResult.stats.regencies.created} new
                  </div>
                  <div>
                    📌 Areas: {uploadResult.stats.areas.created} created,{" "}
                    {uploadResult.stats.areas.updated} updated
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="space-y-2">
                  ...
                  {regencyMode === "existing" ? (
                    <select
                      ...
                      disabled={
                        !selectedLargerState && stateMode === "existing"
                      }
                      className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 disabled:bg-gray-100"
                    >
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {(() => {
                      ...
                      const regencyGroups = Object.values(
                        groupedByRegency,
                      ).sort((a, b) =>
