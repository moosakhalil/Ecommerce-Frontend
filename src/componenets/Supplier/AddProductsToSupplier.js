...
const AddProductsToSupplier = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (data.success) {
        ...
        const mappedProducts = (data.data || data.products || []).map(
          (product) => ({
            _id: product._id,
        ...
        console.log(
          "Mapped products with selectedSupplierId:",
          mappedProducts.filter((p) => p.selectedSupplierId),
        );
  ...
  const handleDeleteRelationship = async (
    ...
    if (
      !window.confirm(
        `Are you sure you want to remove the relationship between "${supplierName}" and "${productName}"?`,
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (supplier && supplier.supplyProducts) {
        const updatedSupplyProducts = supplier.supplyProducts.filter(
          (pid) => pid !== productId,
        );
        ...
        formData.append(
          "supplyProducts",
          JSON.stringify(updatedSupplyProducts),
        );
      ...
      for (const product of products) {
        if (product.selectedSupplierId) {
          const supplier = suppliers.find(
            (s) => s._id === product.selectedSupplierId,
          );
          if (supplier) {
            const key = `${supplier._id}-${product._id}`;
            if (!relationshipMap.has(key)) {
  ...
  const handleSubmitProductsToSupplier = async () => {
    ...
    try {
      ...
      const mergedProducts = [
        ...new Set([...existingProducts, ...newProductIds]),
      ];
  ...
  const handleSubmitSuppliersToProduct = async () => {
    ...
    try {
      ...
      if (supplier) {
        const existingProducts = supplier.supplyProducts || [];
        const mergedProducts = [
          ...new Set([...existingProducts, selectedProduct]),
        ];
        ...
        supplier.supplyProducts.forEach((productId) => {
          ...
          if (product) {
            ...
            productSupplierMap.get(productId).suppliers.push({
              ...
              email: supplier.email,
              deliveryTime:
                supplier.manualAverageDeliveryTime || "Not specified",
            });
          }
        });
      }
      ...
      <div className="flex-1 transition-all duration-300 ml-80 p-6">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-3xl font-semibold mb-6 text-gray-800">
            <strong>84.</strong> Manage Product-Supplier Relationships
          </h1>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            {activeTab === "productsToSupplier" ? (
                    ...
                    <h3 className="text-lg font-semibold text-gray-800 mb-4">
                      Products Already Assigned to This Supplier (
                      {
                        allRelationships.filter(
                          (rel) => rel.supplier._id === selectedSupplier,
                        ).length
                      }
                      )
                    </h3>
                ...
                {selectedProduct && (
                  <div className="mt-8 pt-6 border-t border-gray-200">
                    <h3 className="text-lg font-semibold text-gray-800 mb-4">
                      ...
                      {
                        allRelationships.filter(
                          (rel) => rel.product._id === selectedProduct,
                        ).length
                      }
                      )
                    </h3>
              ...
              <div>
                ...
                {(() => {
                  const suppliersWithNoProducts = suppliers.filter(
                    (supplier) => {
                      // Check if supplier has no products in supplyProducts array
                      const hasSupplyProducts =
                      ...
                      const hasProductsReferencingSupplier = products.some(
                        (product) =>
                          product.selectedSupplierId === supplier._id,
                      );

                      // Supplier has no products if both conditions are false
                      return (
                        !hasSupplyProducts && !hasProductsReferencingSupplier
                      );
                    },
                  );
                  ...
                  return suppliersWithNoProducts.length > 0 ? (
                    <div>
                      <div className="mb-4 text-sm text-gray-600">
                        Found {suppliersWithNoProducts.length} supplier(s) with
                        no products
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                {(() => {
                  const productsWithNoSupplier = products.filter((product) => {
                    ...
                    const hasSuppliersReferencingProduct = suppliers.some(
                      (supplier) =>
                        supplier.supplyProducts &&
                        supplier.supplyProducts.includes(product._id),
                    );

                    // Product has no supplier if both conditions are false
                    return (
                      !hasSelectedSupplier && !hasSuppliersReferencingProduct
                    );
                  });
                  ...
                  return productsWithNoSupplier.length > 0 ? (
                    <div>
                      <div className="mb-4 text-sm text-gray-600">
                        Found {productsWithNoSupplier.length} product(s) with no
                        supplier
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600 mb-6">
                  Select a favorite supplier for products that have multiple
                  suppliers assigned
                </p>
                ...
                {getProductsWithMultipleSuppliers().length > 0 ? (
                  <div>
                    <div className="mb-4 text-sm text-gray-600">
                      Found {getProductsWithMultipleSuppliers().length}{" "}
                      product(s) with multiple suppliers
                    </div>
                    <div className="space-y-6">
                      {getProductsWithMultipleSuppliers().map((item) => {
                        const currentFav = item.product.selectedSupplierId
                          ? String(item.product.selectedSupplierId)
                          : null;
                        const favSupplier = currentFav
                          ? item.suppliers.find(
                              (s) => String(s._id) === currentFav,
                            )
                          : null;
                              ...
                              <tbody className="bg-white divide-y divide-gray-200">
                                {item.suppliers.map((sup) => (
                                  ...
                                  >
                                    ...
                                    <td className="px-4 py-3 text-sm text-gray-600">
                                      {sup.deliveryTime || "N/A"}
                                    </td>
                                    <td className="px-4 py-3">
                                      {currentFav === String(sup._id) ? (
                                        <span className="px-3 py-1 text-sm rounded font-medium bg-gray-200 text-gray-500">
                                          ✓ Selected
                                        </span>
                                      ) : (
                                        <button
                                          ...
                                          onClick={async () => {
                                            ...
                                            try {
                                              const res = await fetch(
                                                `${API_BASE_URL}/api/products/${item.product._id}/supplier`,
                                                {
                                                  method: "PUT",
                                                  headers: {
                                                    "Content-Type":
                  ...
                  <div className="text-center py-8 bg-gray-50 rounded-lg">
                    ...
                    <p className="text-sm text-gray-400 mt-2">
                      Assign multiple suppliers to products first.
                    </p>
                  </div>
                )}
              </div>
