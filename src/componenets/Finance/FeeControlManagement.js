...
const FEE_TYPES = [
  ...
  {
    key: "multiple_item_discount",
    label: "Multiple Item Discount",
    category: "discount",
    icon: Tag,
  },
  {
...
export default function FeeControlManagement() {
  ...
  const fetchReasonCodes = async () => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/fee-control/reason-codes`,
      );
  ...
  const handleAddReasonCode = async () => {
    if (!newReasonCode.code || !newReasonCode.description) {
      setMessage({
        type: "error",
        text: "Please fill in code and description",
      });
      return;
    }
  ...
  const handleSaveReasonCode = async () => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/fee-control/reason-codes/${editingReasonId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
      ...
      if (data.success) {
        setReasonCodes(
          reasonCodes.map((r) =>
            r._id === editingReasonId ? data.reasonCode : r,
          ),
        );
        setEditingReasonId(null);
        setMessage({
          type: "success",
          text: "Reason code updated successfully",
        });
      } else {
        setMessage({ type: "error", text: data.message });
      }
  ...
  const handleDeleteReasonCode = async (id) => {
    if (!window.confirm("Are you sure you want to delete this reason code?"))
      return;

    try {
      const response = await fetch(
        `${API_BASE_URL}/api/fee-control/reason-codes/${id}`,
        {
          method: "DELETE",
        },
      );
      ...
      if (data.success) {
        ...
        setMessage({
          type: "success",
          text: "Reason code deleted successfully",
        });
      } else {
        setMessage({ type: "error", text: data.message });
      }
  ...
  const fetchFees = async () => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/fee-control/fees/standalone/all`,
      );
      const data = await response.json();
      if (data.success) {
        setFees(data.fees || []);
  ...
  const handleAddFee = async () => {
    if (!newFee.feeType || !newFee.amount) {
      setMessage({
        type: "error",
        text: "Please select fee type and enter amount",
      });
      return;
    }
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/fee-control/fees/standalone`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteFee = async (feeId) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/fee-control/fees/${feeControlId}/fee/${feeId}`,
        {
          method: "DELETE",
        },
      );
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: "24px" }}>
          ...
          >
            <strong>265.</strong> 💰 Fee Control Management
          </h1>
                        ...
                        {editingReasonId === reason._id ? (
                          ...
                          <>
                            ...
                            <td style={tableCellStyle}>
                              ...
                              >
                                {REASON_CATEGORIES.find(
                                  (c) => c.value === reason.category,
                            ...
                            >
                              ...
                              <button
                                onClick={() =>
                                  handleDeleteReasonCode(reason._id)
                                }
                                style={{
                                  ...buttonStyle,
          ...
          <div>
            ...
            >
              ...
              <div
                style={{
                  display: "grid",
                  gridTemplateColumns: "1.5fr 1fr 1.5fr 2fr auto",
                  gap: "12px",
                  alignItems: "flex-end",
                }}
              >
                <div>
                  ...
                  >
                    ...
                    <optgroup label="Discounts">
                      {FEE_TYPES.filter((f) => f.category === "discount").map(
                        (fee) => (
                    ...
                    <optgroup label="Fees">
                      {FEE_TYPES.filter((f) => f.category === "fee").map(
                        (fee) => (
                          <option key={fee.key} value={fee.key}>
                            {fee.label}
                          </option>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewFee({ ...newFee, reasonCodeId: e.target.value })
                    }
                    style={inputStyle}
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    value={newFee.note}
                    onChange={(e) =>
                      setNewFee({ ...newFee, note: e.target.value })
                    }
                      ...
                      <tr key={fee._id}>
                        <td style={tableCellStyle}>
                          <span
                            style={{
                              backgroundColor:
                                FEE_TYPES.find((f) => f.key === fee.feeType)
                                  ?.category === "discount"
                                  ? "#fef3c7"
                                  : "#dbeafe",
                              color:
      ...
      {showModeToggle && (
        <ModeToggle
          componentName="Fee Control Management"
