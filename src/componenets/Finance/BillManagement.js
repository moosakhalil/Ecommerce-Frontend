          ...
          <div className="p-8 flex-1">
            ...
            <div className="flex justify-between mb-8">
              ...
              <div className="text-right">
                <div className="mb-2">
                  <span className="text-gray-600 block text-xs uppercase tracking-wide">Invoice Number</span>
                  <span className="font-bold text-gray-800">{bill.orderId}</span>
                </div>
                <div className="mb-2">
                  <span className="text-gray-600 block text-xs uppercase tracking-wide">Date</span>
                  <span className="font-bold text-gray-800">{formatDate(bill.date)}</span>
                </div>
                <div>
                  <span className="text-gray-600 block text-xs uppercase tracking-wide">Status</span>
                  <span className={`px-2 py-1 rounded text-xs font-bold inline-block mt-1 ${
                    bill.status === "pending" ? "bg-yellow-100 text-yellow-800" :
                    bill.status.includes("paid") || bill.status === "completed" ? "bg-green-100 text-green-800" :
            ...
            <div className="flex justify-end mb-8">
              <div className="w-64">
                ...
                <div className="flex justify-between py-4 border-t-2 border-gray-800 mt-2">
                  ...
                  <span className="text-xl font-bold text-indigo-600">{formatCurrency(bill.totalAmount)}</span>
                </div>
              </div>
            </div>
            ...
            {bill.receiptImage ? (
               <div className="mt-8 pt-8 border-t border-gray-200 break-inside-avoid">
                 <h3 className="text-lg font-bold text-gray-800 mb-4">Payment Receipt / Proof</h3>
                 <div className="border rounded-lg p-2 bg-gray-50 inline-block">
                    ...
                    <img 
                      src={(() => {
                        ...
                        const rawData = img?.base64Data || img?.data || (typeof img === 'string' ? img : '');
                        
                        if (!rawData || typeof rawData !== 'string') return '';
                        
                        // If it's already a URL or Data URI, return it
                        if (rawData.startsWith('http') || rawData.startsWith('data:')) return rawData;
              ...
              <div className="mt-8 pt-8 border-t border-gray-200">
                <p className="text-gray-500 italic text-sm">No payment receipt image available for this record.</p>
              </div>
            )}
          </div>
        ...
        <div className="flex-1 overflow-auto">
          <div className="p-8">
            <div className="mb-6 flex justify-between items-center">
              <div>
                <h1 className="text-2xl font-bold text-gray-800">Bill Management & History</h1>
                <p className="text-gray-600">View and manage all customer invoices and receipts</p>
              </div>
              <div className="bg-white px-4 py-2 rounded-lg shadow-sm border">
                 <span className="text-sm text-gray-500">Total Bills: </span>
                 <span className="font-bold text-indigo-600">{bills.length}</span>
              </div>
            </div>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {bills.length > 0 ? (
                        bills.map((bill) => (
                          <tr key={bill.orderId} className="hover:bg-gray-50 transition-colors">
